"""
Парсер каталога опор.
https://alterv.ru/catalog/opory/
Включает колонку "Путь" — breadcrumbs со страницы товара.
"""
...
def get_product_links(page):
    ...
    print(f"  Стр.1: товаров={len(page.query_selector_all('.catalog_item_wrapp'))}")
    ...
    for pg_url in pagination_urls:
        ...
        _hide_popup(page)
        _click_show_more(page)
        print(f"    товаров={len(page.query_selector_all('.catalog_item_wrapp'))}")
        _collect_links(page, seen, links)
...
def get_breadcrumbs(html: str) -> str:
    ...
    parts = []
    # Берём все bx-breadcrumb-item — из каждого только itemprop="name" span
    ...
    # Последний элемент — просто span (название товара, не в div)
    last = nav.find_all("span", recursive=False)
    for s in last:
...
def main():
    ...
    with sync_playwright() as p:
        ...
        print("\n--- Проход 1: сбор заголовков и данных ---")
        all_cols_ordered = []
        seen_cols = set()
