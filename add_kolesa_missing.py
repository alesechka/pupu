"""Добавляет 3 недостающих товара в alterv_kolesa.csv и делает скрины."""
...
MISSING_URLS = [
    "https://alterv.ru/catalog/kolesa_bez_kronshteyna/mkr_kolesa_iz_polipropilena_dlya_krepleniya_na_os_s_rezinovoy_shinoy/",
    "https://alterv.ru/catalog/kolesa_s_tsapfoy/em07spr_kolesa_promyshlennye_povorotnye_s_tsapfoy/",
    "https://alterv.ru/catalog/kolesa_bez_kronshteyna/tskrh_kolesa_iz_listovoy_stali_dlya_krepleniya_na_os/",
]
BASE_URL = "https://alterv.ru"
CSV_FILE = "alterv_kolesa.csv"
SCREENSHOT_DIR = "table_screenshots_kolesa"
            ...
            try:
                ...
                for table in tables:
                    ...
                    for th in thead.find_all("th"):
                        ...
                        if name and name != "Заказать" and name not in all_cols:
                            print(f"  + новая колонка: {name}")
                ...
                for table in tables:
                    ...
                    for tr in (tbody or table).find_all("tr", class_="table_row"):
                        ...
                        row = [title, url, brand, breadcrumb, main_imgs, extra_imgs, prim_html]
                        for col_name in all_cols:
                            row.append(get_cell(col_index.get(col_name, -1)))
                        new_rows.append(row)
                print(f"  Строк добавлено: {len([r for r in new_rows if r[1]==url])}")
