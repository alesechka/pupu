"""
Скриншоты таблиц flt-table для каталога замков поворотных.
- Убирает "Сбросить фильтр"
- Обрезает колонки начиная с "Наличие" и правее
- Убирает "В избранное" и сердечко
Результат в папке table_screenshots_zamki/
"""
...
OUT_DIR = "table_screenshots_zamki"
...
def clean_table(table) -> None:
    ...
    # Убираем "В избранное" и сердечко
    ...
    # Обрезаем колонки начиная с "Наличие"
...
def get_product_links(page) -> list[tuple[str, str]]:
...
def screenshot_tables(page, title: str, url: str):
    ...
    for i, table in enumerate(tables, 1):
        ...
        print(f"  [{title}] таблица {i}/{len(tables)} -> {out_path} ({w}x{h}px)")
